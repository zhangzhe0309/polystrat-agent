...
**核心优化:**
- [P0] 优化 `analyze_sentiment_simple` 函数（更多词汇、权重区分、否定词处理）
- [P0] 优化 `analyze_sentiment_with_llm` 函数（更详细的提示词、更好的解析）
- [P1] 扩展关键词列表（从 36 个扩展到 100+ 个）
- [P1] 添加权重区分（强/中/弱情感词有不同权重）
- [P1] 添加否定词处理（not、no、never 等）
- [P1] 添加程度词处理（very、extremely 等）
- [P1] 添加预测市场专用词汇（odds、probability、likelihood 等）

**新增功能：**
- 关键词权重系统（强正面/负面 2.0，中等 1.5，弱 1.0）
- 否定词处理（否定正面词变为负面，否定负面词变为正面）
- 程度词处理（very 1.5x，extremely 2.0x，slightly 0.5x）
- LLM 分析优化（更详细的提示词，提取关键词）
