...
def calculate_llm_model_weights(trades=None, window_days=7):
    ...
    for trade in trades:
        ...
        actual_result = "win" if abs(trade.get("edge", 0)) > 0.03 else "lose"
        ...
        for model_result in model_results:
            if "|" in model_result:
                ...
                if len(parts) >= 2:
                    ...
                    # 判断预测是否正确
                    if actual_direction == "Yes" and "Yes" in prediction:
                        model_stats[model_name]["correct"] += 1
                    elif actual_direction == "No" and "No" in prediction:
                        model_stats[model_name]["correct"] += 1
...
def calculate_news_source_scores(trades=None, window_days=7):
    ...
    # 统计每个新闻源的质量
    source_stats = defaultdict(lambda: {"count": 0, "sentiment_aligned": 0})
    
    for trade in trades:
        sentiment_score = trade.get("sentiment_score", 0)
        final_direction = "Yes" if trade.get("edge", 0) > 0 else "No"
        
        # 这里需要从新闻搜索结果中获取来源信息
        # 由于当前日志没有记录来源，我们使用默认评分
        # 后续可以在 news_search.py 中记录来源统计
...
def format_optimization_report():
    ...
    for model, weight in report.get("dynamic_llm_weights", report["llm_weights"]).items():
    ...
    lines.append(f"💰 价格阈值: {thresholds['min_price']:.0%} - {thresholds['max_price']:.0%}")
