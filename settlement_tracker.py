...
def update_settled_trades():
    ...
    if stats["updated"] > 0 or stats["timeout"] > 0:
        save_trades(trades)
        log.info(
            f"更新完成: {stats['wins']}胜 {stats['losses']}负 | 总PnL: {stats['total_pnl']:+.2f}"
        )
...
def get_settlement_summary():
    ...
    # 计算平均盈利/亏损
    win_pnl = [
        t.get("pnl", 0) for t in trades if t.get("result") == "win" and t.get("pnl")
    ]
    lose_pnl = [
        t.get("pnl", 0) for t in trades if t.get("result") == "lose" and t.get("pnl")
    ]
